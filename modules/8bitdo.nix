{ pkgs, ... }:
{
  ...
  services.udev.packages = [
    (pkgs.writeTextFile {
      ...
      '';
      destination = "/etc/udev/rules.d/99-8bitdo.rules";
    })
  ];
  ...
  systemd.services."8bitdo-ultimate-xinput@" = {
    ...
    serviceConfig = {
      Type = "simple";
      ExecStart = "${pkgs.xboxdrv}/bin/xboxdrv --detach-kernel-driver --mimic-xpad --silent --type xbox360 --device-by-id %I --force-feedback";
    };
  };
}
